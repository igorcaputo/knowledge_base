...
begin
  if Connection.Connected then
    Result := Connection.DbcConnection.GetMetadata.GetDatabaseInfo.GetDatabaseProductName //.{$ifdef zeos7up}GetDatabaseInfo.{$endif}GetDatabaseProductName
  else
...
begin
  ...
  if SameText(Copy(VProtocol, 1, 9), 'interbase')
   ...
   else if SameText(Copy(VProtocol, 1, 6), 'sqlite') then
     Result := TPressSqliteDDLBuilder
